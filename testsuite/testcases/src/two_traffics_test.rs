...
use aptos_forge::{
    success_criteria::{SuccessCriteriaChecker, SuccessCriteria},
...
impl NetworkLoadTest for TwoTrafficsTest {
    fn test(
        ...
        SuccessCriteriaChecker::check_core_for_success(&self.inner_success_criteria, report, &rate, Some("inner traffic".to_string()))?;
        Ok(())
    }
}
