...
impl SuccessCriteriaChecker {
    pub fn check_core_for_success(
        ...
        let traffic_name_addition = traffic_name.map(|n| format!(" for {}", n)).unwrap_or_else(|| "".to_string());
        Self::check_tps(success_criteria.min_avg_tps, stats_rate, &traffic_name_addition)?;
        Self::check_latency(&success_criteria.latency_thresholds, stats_rate, &traffic_name_addition)?;
        Ok(())
    }
    ...
    pub async fn check_for_success(
        ...
        Self::check_latency(&success_criteria.latency_thresholds, &stats_rate, &"".to_string())?;
    ...
    pub fn check_tps(min_avg_tps: usize, stats_rate: &TxnStatsRate, traffic_name_addition: &String) -> anyhow::Result<()> {
